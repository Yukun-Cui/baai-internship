#!/usr/bin/env python3
"""Extract failed Enflame (燧原 / Suiyuan) operator names from the Excel results spreadsheet.

This script reads the national-GPU operator test spreadsheet and outputs the list of
operators that failed accuracy tests or were skipped on the Enflame (燧原) backend,
suitable for use with `orchestrator.py --enflame`.

The Enflame result column is auto-detected by matching header text containing
"燧原" (and optionally "结果"/"测试结果"). Override with --result-col if needed.
"""
...
def extract_failed_ops(excel_path: str, output_path: str, sheet_name: str = None,
                       result_col: int = None):
    """Extract Enflame-failed operator names from Excel spreadsheet.

    An operator is considered "failed" if its Enflame result column is:
    ...
    for i, h in enumerate(headers):
        ...
        if "算子" in h_str and "名称" in h_str:
            ...
            # Prefer a column that also mentions a result, else take the first 燧原 column.
    ...
    if result_col is None:
        print("Error: Could not find Enflame (燧原) result column in header.")
        print(f"Headers found: {headers}")
...
def main():
    ...
    default_output = os.path.join(script_dir, "ops_list_enflame.txt")

    parser = argparse.ArgumentParser(
        description="Extract failed Enflame (燧原) operators from Excel spreadsheet"
    )
    ...
    parser.add_argument("--result-col", type=int, default=None,
                        help="0-based index of the Enflame result column (default: auto-detect)")
