...
# 摩尔线程 skill 默认复用 flaggems-pr-submit/data 下的共享数据（规范名 + 待提交列表），
# 也可用本 skill 自己的 data/ 或环境变量覆盖。
_LOCAL_DATA = os.path.join(SKILL_DIR, "data")
_SHARED_DATA = os.path.join(SKILL_DIR, "..", "flaggems-pr-submit", "data")


def _resolve(env_key, filename):
    override = os.environ.get(env_key)
    if override:
        return override
    local = os.path.join(_LOCAL_DATA, filename)
    if os.path.isfile(local):
        return local
    return os.path.join(_SHARED_DATA, filename)
