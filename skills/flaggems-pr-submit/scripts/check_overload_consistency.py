...
示例:
    ...
    python check_overload_consistency.py eq --repo-dir /path/to/FlagGems
...
def check_consistency(yaml_ids, bench_info, test_info, op_id):
    ...
    for yaml_id in sorted(yaml_ids):
        # pytest marker 名不能以下划线开头：前导下划线 id 的 mark 用 underscore 前缀。
        expected_mark = id_to_mark(yaml_id)
        label = yaml_id if expected_mark == yaml_id else f"{yaml_id} → mark {expected_mark}"
        print(f"\n  yaml id: {Colors.BOLD}{label}{Colors.END}")

        # Benchmark mark
        bench_funcs_with_mark = [
            info for info in bench_info if expected_mark in info["marks"]
        ]
        if bench_funcs_with_mark:
            func_names = [f["func_name"] for f in bench_funcs_with_mark]
            ok(f"benchmark @pytest.mark.{expected_mark} → {', '.join(func_names)}")
        else:
            errors.append(
                f"yaml id '{yaml_id}' 在 benchmark 中无对应 @pytest.mark.{expected_mark}"
            )
            fail(f"benchmark 缺少 @pytest.mark.{expected_mark}")
        ...
        test_funcs_with_mark = [
            info for info in test_info if expected_mark in info["marks"]
        ]
        if test_funcs_with_mark:
            func_names = [f["func_name"] for f in test_funcs_with_mark]
            ok(f"test @pytest.mark.{expected_mark} → {', '.join(func_names)}")
        else:
            if yaml_id != op_id:
                warnings.append(
                    f"yaml id '{yaml_id}' 在 test 中无对应 mark（建议对齐）"
                )
                warn(f"test 缺少 @pytest.mark.{expected_mark}（建议对齐）")
            else:
                errors.append(
                    f"yaml id '{yaml_id}' 在 test 中无对应 @pytest.mark.{expected_mark}"
                )
                fail(f"test 缺少 @pytest.mark.{expected_mark}")
...
def main():
    ...
    # 冲突场景下 op_id 保留前导下划线（文件名/yaml id 用）。
    op_id, _mark_id = resolve_op_names(repo, op_name)
