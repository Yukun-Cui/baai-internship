...
from paths import RECORD_PATH, resolve_op_names
...
def get_op_id(op_name, repo_dir=None):
    """算子 id（文件名 / yaml id 用）。

    默认去掉前导下划线；冲突场景（存在裸算子）下保留前导下划线。
    repo_dir 为 None 时退回纯 lstrip（无法探测冲突）。
    """
    if repo_dir is not None:
        op_id, _mark_id = resolve_op_names(repo_dir, op_name)
        return op_id
    return op_name.lstrip("_")


def get_changed_files(op_name, repo_dir, upstream_ref="upstream/infra-ci"):
    op_id = get_op_id(op_name, repo_dir)
    files = [
    ...
    for relpath in optional_files:
        ...
        diff = subprocess.run(
            ["git", "diff", "--name-only", f"{upstream_ref}...HEAD", "--", relpath],
            cwd=repo_dir, capture_output=True, text=True,
        )
...
def main():
    ...
    upstream_ref = f"{target.upstream_remote}/{target.base}"
    ...
    print(f"仓库: {repo}")
    print(f"目标: {target.name} → {target.upstream_repo} (base={target.base})")
    print(f"GPU: {gpu}")
    print()

    files = get_changed_files(op, repo, upstream_ref)
    if len(files) < 6:
    ...
    if args.dry_run:
        ...
        ok(f"push 前 fetch 最新 {upstream_ref} 并最终运行 check_operator.py (--strict)")
        run(["git", "fetch", target.upstream_remote, target.base], cwd=repo, timeout=60)
        run(
            ["python", os.path.join(SCRIPTS_DIR, "check_operator.py"), op,
             "--repo-dir", repo, "--strict", "--target", target.name],
            cwd=repo,
    ...
    # ── Step 7: git push ──
    step(7, "git push")
    branch = f"{target.branch_prefix}{op}"
    if args.dry_run:
        print(f"  ⚠ DRY RUN — 跳过 push 到 {branch}")
    else:
        run(["git", "push", target.push_remote, f"HEAD:{branch}"], cwd=repo, timeout=60)
        ok(f"已推送到 {target.push_remote}/{branch}")
    ...
    if args.dry_run:
        ...
        result = run(
            [
                "gh", "api", f"repos/{target.upstream_repo}/pulls",
                "-f", f"title={title}",
                "-f", f"head={target.fork_owner}:{branch}",
                "-f", f"base={target.base}",
                "-f", f"body={body}",
            ],
            env={"GH_TOKEN": token},
            capture=True,
